 ...
 *    Nicolas Daitsch, Nro de Puerto publico y correccion en "$receiveddone" cuando viene vacio. (ndaitsch [at] gmail [dot] com)
 *
                ...
                if ($RESPONSE[0] == '!done') {
                    if (preg_match_all('/[^=]+/i', $RESPONSE[1], $MATCHES)) {
                        if ($MATCHES[0][0] == 'ret' && strlen($MATCHES[0][1]) == 32) {
                            ...
                            $RESPONSE = $this->read(false);
                            if (!empty($RESPONSE[0]) && $RESPONSE[0] == '!done') {
                                $this->connected = true;
                                break;
                            }
                        }
        ...
        while (true) {
            ...
            if ($LENGTH > 0) {
                ...
                while ($retlen < $LENGTH) {
                    ...
                    $underscore .= fread($this->socket, $toread);
                    $retlen = strlen($underscore);
                }
                $RESPONSE[] = $underscore;
                $this->debug('>>> [' . $retlen . '/' . $LENGTH . '] bytes read.');
            }
            // If we get a !done, make a note of it.
            if (!empty($underscore) && $underscore == "!done")
                $receiveddone = true;
            $STATUS = socket_get_status($this->socket);
            if ($LENGTH > 0)
                $this->debug('>>> [' . $LENGTH . ', ' . $STATUS['unread_bytes'] . ']' . $underscore);
            if ((!$this->connected && !$STATUS['unread_bytes']) || ($this->connected && !$STATUS['unread_bytes'] && !empty($receiveddone)))
                break;
        }
        if ($parse)
